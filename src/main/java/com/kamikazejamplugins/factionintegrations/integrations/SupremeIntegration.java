...
import com.kamikazejamplugins.factionintegrations.shield.ShieldIntegration;
...
//AL56AF50
public class SupremeIntegration implements KFaction, ShieldIntegration {
    ...
    private Method isBaseRegion;

    private Method getShieldSlot;

    private Method isProtectedNow;
    ...
    public SupremeIntegration() throws ClassNotFoundException, NoSuchMethodException, NoSuchFieldException, IllegalAccessException, InvocationTargetException {
        ...
        getShieldSlot = factionClass.getDeclaredMethod("getShieldSlot");

        isProtectedNow = Class.forName("com.massivecraft.factions.zcore.supreme.shield.ShieldSlot").getDeclaredMethod("isProtectedNow");

        getMaxTnT = factionClass.getMethod("getMaxTNT");

        isBaseRegion = factionClass.getMethod("isBaseRegion", FLocation.class);
        ...
        Class<?> pClass = Class.forName("com.massivecraft.factions.P");

        this.p = pClass.getField("p").get(null);

        sotw = pClass.getField("sotw");
    ...
    public int addTnT(String id, int amount) {
        try{
            Faction faction = Factions.getInstance().getFactionById(id);
            int totalAmount = ((Double) getBalance.invoke(faction)).intValue();
    ...
    public int getTnT(String id) {
        try{
            Faction faction = Factions.getInstance().getFactionById(id);
            return ((Double) getBalance.invoke(faction)).intValue();
        }catch(IllegalAccessException | InvocationTargetException e){
    ...
    public int getMaxTnt(String id) {
        try {
            double floor = Math.floor((double) getMaxTnT.invoke(Factions.getInstance().getFactionById(id)));
            return (int) floor;
        }catch (IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
        }
    ...
    public boolean isBaseRegion(Location location, String id) {
        ...
        if (!id.equalsIgnoreCase(faction.getId()))
            return false;
        try {
            return (Boolean) this.isBaseRegion.invoke(faction, fLocation);
    ...
    public boolean isShieldActiveNow(String id) {
        ...
        try {
            Object shieldSlot = getShieldSlot.invoke(faction);

            if (shieldSlot == null) return false;

            return (boolean) isProtectedNow.invoke(shieldSlot);
        }catch (IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
        }
        return false;
    ...
    public void setPermission(String id, TranslatedRelation relation, String permission, boolean b) {
        ...
        try{
            ...
            try{
                Enum<?> en = (Enum<?>) valueOf.invoke(null, permission);
