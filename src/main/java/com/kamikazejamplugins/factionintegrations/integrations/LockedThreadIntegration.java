...
import com.kamikazejamplugins.factionintegrations.event.*;
...
public class LockedThreadIntegration implements KFaction {
    ...
    private Method isSotw;
    private Method isLocked;

    private Method getRole;

    private Method getOnlinePlayers;

    private Method getFactionChest;
    ...
    private Method getRelationTo;

    private Field leaderP, coleaderP, moderatorP, normalP, recruitP;
    public LockedThreadIntegration() throws ClassNotFoundException, NoSuchMethodException, NoSuchFieldException, IllegalAccessException, InvocationTargetException {
        Class<?> factionClass = Class.forName("com.massivecraft.factions.Faction");

        Class<?> mainClass = Class.forName("com.massivecraft.factions.P");

        getLimit = factionClass.getMethod("getTntBankLimit");

        set = factionClass.getMethod("setTntBankBalance", int.class);

        getFactionChest = factionClass.getMethod("getFactionChest");

        getInventory = Class.forName("com.massivecraft.factions.zcore.factionchest.FactionChest").getMethod("getInventory");

        getBalance = factionClass.getMethod("getTntBankBalance");
        ...
        leaderP = confClass.getField("prefixAdmin");
        ...
        isSotw = mainClass.getMethod("isSotw");

        isLocked = mainClass.getMethod("getLocked");

        getStrikes = factionClass.getMethod("getStrikes");

        maxMembers = factionClass.getMethod("getMaxMembers");

        Class<?> aClass = Class.forName("com.massivecraft.factions.iface.RelationParticipator");

        getRelationTo = aClass.getMethod("getRelationTo", aClass);

        main = mainClass.getField("p").get(null);
    ...
    public void setTnT(String id, int amount) {
        try{
            ...
            if(amount > limit){
                set.invoke(faction, limit);
                return;
            }
            set.invoke(faction, amount);
    ...
    public int addTnT(String id, int amount) {
        try{
            ...
            int limit = (int) getLimit.invoke(faction);
            int allowedRoom = limit - totalAmount;
            if(amount + totalAmount > limit){
                if(amount == totalAmount)return 0;
                set.invoke(faction, limit);
                return allowedRoom;
            }
            set.invoke(faction, amount + totalAmount);
            return amount;
        }catch(IllegalAccessException | InvocationTargetException e){
            Bukkit.getLogger().severe("Error setting tnt bank balance!");
        }
    ...
    public int getStrikes(String id) {
        try{
            ...
            Object thing = getStrikes.invoke(faction);

            if(thing == null)return 0;

            return ((Set<Object>) thing).size();
    ...
    public boolean isSystemFac(String id) {
        Faction checkFaction = Factions.getInstance().getFactionById(id);

        if(checkFaction == null)return false;

        return checkFaction.isSafeZone() || checkFaction.isWilderness() || checkFaction.isWarZone();
    }
    ...
    public boolean isSOTW() {
        try{
            return (boolean) isSotw.invoke(main);
        } catch (IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
        }
        return false;
    ...
    public boolean isSystemFacAt(Location location) {
        FLocation checkLoc = new FLocation(location);
        Faction checkFac = Board.getInstance().getFactionAt(checkLoc);
        if(checkFac == null)return false;
        return checkFac.isSafeZone() || checkFac.isWarZone() || checkFac.isWilderness();
    }

    @Override
    public boolean isLocked() {
        try{
            return (boolean) isLocked.invoke(main);
        } catch (IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
        }
        return false;
    ...
    public int getMaxPlayers(String id) {
        try{
            return (int) maxMembers.invoke(Factions.getInstance().getFactionById(id));
    ...
    public boolean isInFactionChest(Player player, String id) {
        ...
        try{
            Object factionChest = getFactionChest.invoke(faction);

            return ((Inventory) getInventory.invoke(factionChest)).getViewers().contains(player);
    ...
    public Inventory getChestInventory(Player player, String id) {
        ...
        try{
            ...
            return (Inventory) getInventory.invoke(factionChest);
        } catch (IllegalAccessException | InvocationTargetException ignored) {
    ...
    public String getFactionsIdAt(Location location) {
        FLocation fLocation = new FLocation(location);
        return Board.getInstance().getFactionAt(fLocation).getId();
    }
